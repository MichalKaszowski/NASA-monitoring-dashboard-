...
export function TopDiameterAsteroidsChart({ asteroids }: ChartProps) {

    const topAsteroids  =[...asteroids].sort((a, b) => {
    ...
    return (
        <Box sx={{backgroundColor: 'lightgray', borderRadius: '5px'}}>
            <Typography margin={2}>Top 5 largest asteroids (by max est. diameter) in comparison to some popular buildings </Typography>
            <BarChart
                dataset={[...topAsteroids, ...COMPARISION_BUILDINGS].sort((a, b) => {
                    const sizeA = 'building_height' in a
                        ? a.building_height
                        : a.estimated_diameter_meters_estimated_diameter_max;

                    const sizeB = 'building_height' in b
                        ? b.building_height
                ...
                series={[{ dataKey: 'estimated_diameter_meters_estimated_diameter_max', label: 'Asteroid' },
                { dataKey: 'building_height', label: 'Building', stackOrder: 'descending' }
                ]}
                {...chartSetting}
            />
        </Box>
